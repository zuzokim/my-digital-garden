...
최근 참여하고 있는 프론트엔드 코드리뷰 스터디에서 간단한 토이프로젝트를 진행하고 있다. 장바구니 기능이 있는 이커머스 페이지에서 상태관리에 초점을 맞추어 기능을 구현하고 서로의 코드를 리뷰해주는 시간을 갖는다. 
...
Zustand의 `combine` 미들웨어 함수 자체 구현 코드가 매우 짧아서 놀랐다. 짧고 단순하지만 타입추론 방식이 Generic을 활용한 방식이라 조금 더 들여다 보고 이해하는 시간이 필요했다.
